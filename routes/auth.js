...
router.post('/student', [
    // Validations
    check('regnum', 'Please enter a valid registration number').isLength({min: 7}),
    ...
    try {
        ...
        if(!student){
            return res.status(400).json({ msg: 'Invalid registration number or password !student' });
        }
        const isMatch = await bcrypt.compare(password, student.password);
        if(!isMatch){
            return res.status(400).json({ msg: 'Invalid registration number or password !isMatch' });
        }
...
router.post('/admin', [
    ...
    try {
        ...
        if(!admin){
            return res.status(400).json({ msg: 'Invalid email or password !admin' });
        }
        const isMatch = await bcrypt.compare(password, admin.password);
        if(!isMatch){
            return res.status(400).json({ msg: 'Invalid email or password !matched' });
        }
